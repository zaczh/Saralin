//
//  SAUserNoticeViewController.swift
//  Saralin
//
...
class SAUserNoticeViewController: SABaseTableViewController {
    ...
    override func viewDidLoad() {
        ...
        refreshTableViewCompletion { [weak self] (result, error) in
            self?.handleTableLoadingResult(result, error: error)
        }
    }
    ...
    override func viewThemeDidChange(_ newTheme:SATheme) {
        ...
        tableView.separatorColor = newTheme.tableCellSeperatorColor.sa_toColor()
    }
    
    override func refreshTableViewCompletion(_ completion: ((SALoadingViewController.LoadingResult, NSError?) -> Void)?) {
        let group = DispatchGroup()
        guard let url0 = URL(string: "home.php?mod=space&do=notice&view=mypost&type=post", relativeTo: URL(string: SAGlobalConfig().forum_base_url)!) else {
            fatalError()
        }
        ...
        URLSession.saCustomized.dataTask(with: url0, completionHandler: { (data, response, error) in
            ...
            parser.body()?.findChildTags("dl").forEach({ (dl) in
        ...
        guard let url1 = URL(string: "home.php?mod=space&do=notice&view=mypost&type=at", relativeTo: URL(string: SAGlobalConfig().forum_base_url)!) else {
            fatalError()
        }
        
        group.enter()
        UIApplication.shared.showNetworkIndicator()
        URLSession.saCustomized.dataTask(with: url1, completionHandler: { (data, response, error) in
            UIApplication.shared.hideNetworkIndicator()
            defer {
                group.leave()
            }
        ...
        group.notify(queue: DispatchQueue.main) {
            if !result {
                let error = NSError.init(domain: SAGeneralErrorDomain, code: -1, userInfo: ["msg":"Request failed"])
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if segmentedControl.selectedSegmentIndex == 0 {
            ...
            attributedString.append(NSAttributedString(string: replierName, attributes: [NSAttributedString.Key.font: font, NSAttributedString.Key.foregroundColor: Theme().htmlLinkColor.sa_toColor()]))
            attributedString.append(NSAttributedString(string: " 在帖子 \(threadTitle) 中回复了你，点击查看。", attributes: [NSAttributedString.Key.font: font, NSAttributedString.Key.foregroundColor: Theme().textColor.sa_toColor()]))
            ...
            let attributedString = NSMutableAttributedString()
            attributedString.append(NSAttributedString(string: replierName, attributes: [NSAttributedString.Key.font: font, NSAttributedString.Key.foregroundColor: Theme().htmlLinkColor.sa_toColor()]))
            attributedString.append(NSAttributedString(string: " 在帖子 \(threadTitle) 中提到了你\n", attributes: [NSAttributedString.Key.font: font, NSAttributedString.Key.foregroundColor: Theme().textColor.sa_toColor()]))
            attributedString.append(NSAttributedString(string: "\(quote)\n", attributes: [NSAttributedString.Key.font: font, NSAttributedString.Key.foregroundColor: Theme().tableCellGrayedTextColor.sa_toColor()]))
            attributedString.append(NSAttributedString(string: "点击查看", attributes: [NSAttributedString.Key.font: font, NSAttributedString.Key.foregroundColor: Theme().textColor.sa_toColor()]))
            cell.customTitleLabel.attributedText = attributedString
            cell.customTimeLabel.text = time
            return cell
        }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if segmentedControl.selectedSegmentIndex == 0 {
            ...
            if let url = URL(string: threadLink, relativeTo: URL(string: SAGlobalConfig().forum_base_url)!), let vc = SAContentViewController.viewControllerForURL(url: url, sender: self) {
            ...
            if let url = URL(string: viewReplyLink, relativeTo: URL(string: SAGlobalConfig().forum_base_url)!), let vc = SAContentViewController.viewControllerForURL(url: url, sender: self) {
                navigationController?.pushViewController(vc, animated: true)
            }
        }
    }
